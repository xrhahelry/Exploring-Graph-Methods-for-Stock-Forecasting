...
from models.predict2 import main as predict_main_gat
from models.predict3 import main as predict_main_adbl_gcn
from models.predict4 import main as predict_main_adbl_gat
from models.predict5 import main as predict_main_nmb_gcn
from models.predict6 import main as predict_main_nmb_gat
import bcrypt
...
async def read_root(request: Request, model: str = 'GCN', bank: str = 'NABIL'):
    try:
        if bank == 'NABIL':
            if model == 'GCN':
                graphs = predict_main()  # predict1.py (GCN for NABIL)
            elif model == 'GAT':
                graphs = predict_main_gat()  # predict2.py (GAT for NABIL)
            else:
                raise HTTPException(status_code=400, detail='Invalid model specified')
        elif bank == 'ADBL':
            if model == 'GCN':
                graphs = predict_main_adbl_gcn()  # predict3.py (GCN for ADBL
            elif model == 'GAT':
                graphs = predict_main_adbl_gat()  # predict4.py (GAT for ADBL
            else:
                raise HTTPException(status_code=400, detail='Invalid model specified')
        elif bank == 'NMB':
            if model ==  'GCN':
                graphs = predict_main_nmb_gcn()
            elif model == 'GAT':
                graphs = predict_main_nmb_gat()
            else:
                raise HTTPException(status_code=400, detail='Invalid model specified')
