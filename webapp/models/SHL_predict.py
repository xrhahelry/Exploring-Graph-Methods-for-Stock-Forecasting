...
# Load the scaler
scaler = joblib.load("../GNN/scalers/ht_shl.pkl")
...
def main(model_type):
    ...
    test_graphs_path = "../GNN/graphs/test_graphs/ht_shl_test.pt"
    test_graphs = torch.load(test_graphs_path)

    # Load the data
    data_path = "../data/fundamental data/hotel and tourism/SHL.csv"
    ...
    model_path = (
        "../GNN/models/gcn_shl.pth"
        if model_type == "GCN"
        else "../GNN/models/gat_shl.pth"
    )
    model = load_model(model_path, input_size, hidden_size, output_size, model_type)

    # Generate predictions for the test set
    test_predictions_df, test_ground_truth_df = generate_predictions(
        model, test_graphs, model_type, df
    )
